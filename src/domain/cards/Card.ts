...
// Player = untargeted plays (e.g. "gain 1 glory" targets the player).
// Fighter = targeted plays (e.g. "give Guard token" targets a fighter).
export type Target = Player | Fighter;
...
export class Card {
  ...
  zone: CardZone;
  attachedToFighter: Fighter | null;
  revealed: boolean;
  ...
  constructor(
    ...
    gloryValue: number,
    zone: CardZone,
    attachedToFighter: Fighter | null = null,
...
// setup time when the owner (Player) is known.
export type CardFactory = (id: string, owner: Player, zone: CardZone) => Card;

// Constructor type for concrete card classes whose constructor is (id, owner, zone).
export type CardConstructor = new (id: string, owner: Player, zone: CardZone) => Card;
